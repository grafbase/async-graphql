...
pub struct ServerError {
    ...
    #[serde(skip)]
    pub source: Option<Arc<dyn Any + crate::SendAndSyncOrNot>>,
...
impl ServerError {
    ...
    /// # });
    /// ```
    pub fn source<T: Any + crate::SendAndSyncOrNot>(&self) -> Option<&T> {
...
impl Error {
    ...
    pub fn new_with_source(source: impl Display + crate::SendAndSyncOrNot + 'static) -> Self {
        Self {
...
impl<T: Display + crate::SendAndSyncOrNot + 'static> From<T> for Error {
    fn from(e: T) -> Self {
        Self {
            message: e.to_string(),
            source: None,
...
pub enum ParseRequestError {
    ...
    InvalidRequest(Box<dyn std::error::Error + crate::SendAndSyncOrNot>),

    /// The request's files map was invalid.
    #[error("Invalid files map: {0}")]
    InvalidFilesMap(Box<dyn std::error::Error + crate::SendAndSyncOrNot>),
...
where
    E: ErrorExtensions + crate::SendAndSyncOrNot + 'static,
{
    fn extend_err<C>(self, cb: C) -> Result<T>
    where
        C: FnOnce(&E, &mut ErrorExtensionValues),
