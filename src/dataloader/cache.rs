...
pub trait CacheFactory: crate::SendAndSyncOrNot + 'static {
    ...
    where
        K: crate::SendAndSyncOrNot + Clone + Eq + Hash + 'static,
        V: crate::SendAndSyncOrNot + Clone + 'static;
}

/// Cache storage for [DataLoader].
pub trait CacheStorage: crate::SendAndSyncOrNot + 'static {
    /// The key type of the record.
    type Key: crate::SendAndSyncOrNot + Clone + Eq + Hash + 'static;
...
impl<S: crate::SendAndSyncOrNot + BuildHasher + Default + 'static> HashMapCache<S> {
...
impl<S: crate::SendAndSyncOrNot + BuildHasher + Default + 'static> CacheFactory for HashMapCache<S> {
    fn create<K, V>(&self) -> Box<dyn CacheStorage<Key = K, Value = V>>
    where
        K: crate::SendAndSyncOrNot + Clone + Eq + Hash + 'static,
        V: crate::SendAndSyncOrNot + Clone + 'static,
...
where
    K: crate::SendAndSyncOrNot + Clone + Eq + Hash + 'static,
    V: crate::SendAndSyncOrNot + Clone + 'static,
    S: crate::SendAndSyncOrNot + BuildHasher + 'static,
{
    type Key = K;
    type Value = V;
